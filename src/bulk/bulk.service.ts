...
export class BulkService {
  ...
  async bulkSellSetNotBlockchain(
    ...
    await this.saveTokensSearchIndex(collectionId, newTokensSell);
  ...
  async saveTokensSearchIndex(collectionId: number, tokens: string[]) {
    // TODO: change loop to bulk operations if possible
    for (const tokenId of tokens) {
      const tokenInfo = await this.searchIndex.getTokenInfoItems({ collectionId, tokenId: parseInt(tokenId) });
      await this.searchIndex.saveSearchIndex({ collectionId, tokenId: parseInt(tokenId) }, tokenInfo);
    }
  }
}
