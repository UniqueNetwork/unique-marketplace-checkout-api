...
import { initApp, runMigrations, prepareTradesData, sortToString } from './data';
...
describe('Trades service', () => {
  ...
  describe('GET /trades HttpStatus', () => {
    it('should return response status 200', async () => {
      const response = await searchByFilterTradesGet(app, {}, undefined, {
        sort: [{ order: null, column: '' }],
      });
      expect(response.statusCode).toBe(200);
    });

    it('should return response status 200 non-existent sort asc(Test) ', async () => {
      const response = await searchByFilterTradesGet(app, { page: 100 }, undefined, { sort: [{ order: 0, column: 'Test' }] }, [10]);
      expect(response.statusCode).toBe(200);
    });

    it('should return response status 400 (Bad Request) sort page: 0 )', async () => {
      const response = await searchByFilterTradesGet(app, { page: 0, pageSize: 0 }, undefined, {
        sort: [{ order: 0, column: '' }],
      });
  ...
  describe('GET /trades', () => {
    it('should return sort Null with 3 tokens', async () => {
      const response = await searchByFilterTradesGet(app, { page: 1, pageSize: 3 }, undefined, {
        sort: [{ order: null, column: '' }],
      });
      ...
      expect(response.body.items.map((x) => `${x.collectionId}:${x.tokenId}`)).toEqual(mockTokens);
    });

    it('should return sort asc(TradeDate) with 3 tokens', async () => {
      const response = await searchByFilterTradesGet(app, { pageSize: 3 }, undefined, {
        sort: [{ order: 0, column: 'TradeDate' }],
      });
    ...
    it('should return sort desc(CollectionId) with tokens', async () => {
      const response = await searchByFilterTradesGet(app, { pageSize: 5 }, undefined, {
        sort: [{ order: 1, column: 'CollectionId' }],
      });
    ...
    it('should return sort asc(CollectionId) with tokens 10 pages default', async () => {
      const response = await searchByFilterTradesGet(app, {}, undefined, {
        sort: [{ order: 0, column: 'CollectionId' }],
      });
    ...
    it('should return sort desc(Price) and 5 items ', async () => {
      ...
      expect(response.statusCode).toBe(200);
      expect(response.body.items.length).toBe(5);
      expect(response.body.items.map((x) => `${x.collectionId}:${x.price}`)).toEqual(mockPrice);
    });
    ...
    it('should return sort collectionId: 3', async () => {
      const response = await searchByFilterTradesGet(app, {}, undefined, { sort: [{ order: 0, column: '' }] }, [3]);
      expect(response.statusCode).toBe(200);
      expect(response.body.items.length).toBe(0);
    });

    it('should return sort page: 1 pageSize: 2  === length 2', async () => {
      const response = await searchByFilterTradesGet(app, { page: 1, pageSize: 2 }, undefined, {
        sort: [{ order: 0, column: '' }],
      });
    ...
    it('should return 4 items, find by buyer and seller ', async () => {
      const response = await searchByFilterTradesGet(app, { page: 1, pageSize: 20 }, '5ECaAmPWvw9CAf7hi1tncBTcYFKy9pMahQA1LYZ5ET3of9WH', {
        sort: [{ order: 0, column: '' }],
      });

      expect(response.statusCode).toBe(200);
      expect(response.body.items.length).toBe(4);
      expect(response.body.items[0].buyer === '5ECaAmPWvw9CAf7hi1tncBTcYFKy9pMahQA1LYZ5ET3of9WH').toBe(true);
      expect(response.body.items[3].seller === '5ECaAmPWvw9CAf7hi1tncBTcYFKy9pMahQA1LYZ5ET3of9WH').toBe(true);
    });
  });
});
